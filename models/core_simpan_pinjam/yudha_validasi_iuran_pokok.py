...
class yudha_validasi_iuran_pokok(models.Model):
    _name = 'yudha.validasi.iuran.pokok'
    ...
    jns_dok = fields.Char(string='Jenis Dokument', default='Validasi Simpanan Pokok',index=True,copy=True,required=True, readonly=True)
    ...
    iuranp_ids=fields.One2many(comodel_name='yudha.validasi.iuran.pokok.details',inverse_name="valid_id",string='yudha validasi iuran pokok')
    ...
    def _Tampil_data(self,donum):
        if not donum:
            ...
            mysql="""SELECT * FROM yudha_iuran_pokok a WHERE a.date=%s;"""
    ...
    def onchange_tgl_periode(self):
        if not self.tgl_periode:
            return
        self.iuranp_ids = self._Tampil_data(self.tgl_periode)

    
    def unlink(self):
        for line in self.iuranp_ids:
            if line.amount != 0:
                if line.state != 'draft':
                    raise ValidationError(_('Status Confirm tidak bisa dihapus'))
        return super(yudha_validasi_iuran_pokok, self).unlink()
    ...
    def buat_jurnal(self,jumlah,tglbuat):
        ...
        for x in range(0, 2):
            if x == 0:
                myacc = self.env['account.account'].search(['&',('name', '=', 'SIMPANAN POKOK ANGGOTA'),('company_id','=',mycompid.id)])
    ...
    def validate(self):
        if self.state == 'draft':
            for alldata in self.iuranp_ids:
                if alldata.amount == 0:
                    raise ValidationError(_('Jumlah Iuran 0, tidak bisa di validasi'))

            # update saldo iuran pokok di res_partner
            totsaldo = 0
            for allline in self.iuranp_ids:
                res_partner_obj = self.env['res.partner'].search([('id', '=', allline.partner_id.id)])
                iuran_pokok=res_partner_obj.iuran_pokok
                iuran_pokok=iuran_pokok+allline.amount
                res_partner_obj.write({'iuran_pokok': iuran_pokok})
                totsaldo = totsaldo + allline.amount
            self.buat_jurnal(totsaldo,self.tgl_periode)
            self.write({'state': 'confirm'})
            self.state = SESSION_STATES[1][0]


class yudha_validasi_iuran_pokok_details(models.Model):
    _name = 'yudha.validasi.iuran.pokok.details'

    valid_id= fields.Many2one(comodel_name='validasi.timbang1',inverse_name='id', string="Validasi Timbang I Details", required=False,store=True,index=True )
