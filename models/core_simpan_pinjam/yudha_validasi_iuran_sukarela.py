...
class yudha_validasi_iuran_sukarela(models.Model):
    _name = 'yudha.validasi.iuran.sukarela'
    ...
    jns_dok = fields.Char(string='Jenis Dokument', default='Validasi Simpanan Sukarela',index=True,copy=True,required=True, readonly=True)
    ...
    iurans_ids=fields.One2many(comodel_name='yudha.validasi.iuran.sukarela.details',inverse_name="valid_id",string='yudha validasi iuran pokok')
    ...
    def _Tampil_data(self,donum):
        if not donum:
            ...
            mysql="""SELECT * FROM yudha_iuran_sukarela a WHERE a.date=%s;"""
    ...
    def onchange_tgl_periode(self):
        if not self.tgl_periode:
            return
        self.iurans_ids = self._Tampil_data(self.tgl_periode)

    
    def unlink(self):
        for line in self.iurans_ids:
            if line.amount != 0:
                if line.state != 'draft':
                    raise ValidationError(_('Status Confirm tidak bisa dihapus'))
        return super(yudha_validasi_iuran_sukarela, self).unlink()
    ...
    def buat_jurnal(self,jumlah,tglbuat):
        ...
        for x in range(0, 2):
            if x == 0:
                myacc = self.env['account.account'].search(['&',('name', '=', 'SIMPANAN SUKARELA ANGGOTA'),('company_id','=',mycompid.id)])
    ...
    def validate(self):
        if self.state == 'draft':
            for alldata in self.iurans_ids:
                if self.amount == 0:
                    raise ValidationError(_('Jumlah Iuran 0, tidak bisa di validasi'))
            totsaldo = 0
            for alline in self.iurans_ids:
                # update saldo iuran sukarela di res_partner
                res_partner_obj = self.env['res.partner'].search([('id', '=', alline.partner_id.id)])
                iuran_sukarela = res_partner_obj.iuran_sukarela
                iuran_sukarela = iuran_sukarela + alline.amount
                res_partner_obj.write({'iuran_sukarela': iuran_sukarela})
...
class yudha_validasi_iuran_sukarela_details(models.Model):
    _name = 'yudha.validasi.iuran.sukarela.details'

    valid_id= fields.Many2one(comodel_name='yudha.validasi.iuran.sukarela',inverse_name='id', string="Validasi Iuran Sukarela Details", required=False,store=True,index=True )
