...
class yudha_iuran_sukarela(models.Model):
    _name = 'yudha.iuran.sukarela'
    _order = 'docnum desc'
    _description = "yudha IURAN SUKARELA"

    docnum = fields.Char(size=100, string='No. Transaksi',readonly=True)
    nm_trans = fields.Char(size=100, string='Nama Transaksi', default='Simpanan sukarela', readonly=True)
    ...
    iuransuka_ids = fields.One2many(comodel_name='yudha.iuran.sukarela.details', inverse_name='iuransuka_val',string='Iuran sukarela')
    ...
    amount = fields.Float('Jumlah Simpanan Sukarela', digits=(19, 2), default=0)
    ...
    def unlink(self):
        ...
        return super(yudha_iuran_sukarela, self).unlink()
    ...
    def create(self, vals):
        ...
        myquery = """SELECT max(docnum) FROM yudha_iuran_sukarela;"""
        ...
        if no_urut != None or no_urut != False:
            ...
            vals['docnum'] = 'SIMSKR/' + str(tahun2) + '/' + str(nomerurut)
        else:
            vals['docnum'] ='SIMSKR/'+ str(tahun2) + '/' + '1'
        vals['state'] = 'ready'

        return super(yudha_iuran_sukarela, self).create(vals)
...
class yudha_iuran_sukarela_details(models.Model):
    _name = 'yudha.iuran.sukarela.details'

    iuransuka_val = fields.Many2one(comodel_name='yudha.iuran.sukarela', inverse_name='id', string="Iuran sukarela Details",required=False, store=True, index=True)
