...
class yudha_PelunasanKonsumtif(models.Model):
    _name = 'yudha.pelunasan.konsumtif'
    ...
    def default_get(self, fields):
        ...
        konsumtif_obj = self.env['yudha.peminjaman.konsumtif'].search([('id','=',loan_id[0])])
        rec = super(yudha_PelunasanKonsumtif, self).default_get(fields)
        if loan_id:
            rec.update({'loan_id': loan_id[0]})
            rec.update({'lama_cicilan': konsumtif_obj.lama_cicilan})
            rec.update({'sisa_cicilan': konsumtif_obj.sisa_cicilan})
            rec.update({'last_paydate': konsumtif_obj.last_paydate})
            rec.update({'sisa_loan': konsumtif_obj.sisa_loan})
            rec.update({'bunga': konsumtif_obj.bunga})
        return rec
    ...
    def post_pelunasan(self):
        ...
        lunas_obj = self.env['yudha.pelunasan.konsumtif']
        lunas_obj.create({
            'loan_id': loan_id,
        ...
        yudha_obj = self.env['yudha.peminjaman.konsumtif'].search([('id', '=', loan_id)])
        yudha_obj_detail = self.env['yudha.peminjaman.konsumtif.details'].search([('loan_id', '=', loan_id),('cicilan_ke','>',cicilan_ke)])
        ...
        pinjam_obj = self.env['yudha.peminjaman.konsumtif.details']
        pinjam_obj.create({
            'loan_id': loan_id,
            'cicilan_ke': 0,
            'date_pay': date_pay,
        ...
        if partner_obj:
            pinj_konsumtif=float(partner_obj.pinj_konsumtif)-amount
            partner_obj.write({
                'pinj_konsumtif': pinj_konsumtif
            })
