...
class yudha_PelunasanBarang(models.Model):
    _name = 'yudha.pelunasan.barang'
    ...
    def default_get(self, fields):
        ...
        barang_obj = self.env['yudha.peminjaman.barang'].search([('id','=',loan_id[0])])
        rec = super(yudha_PelunasanBarang, self).default_get(fields)
        if loan_id:
            rec.update({'loan_id': loan_id[0]})
            rec.update({'lama_cicilan': barang_obj.lama_cicilan})
            rec.update({'sisa_cicilan': barang_obj.sisa_cicilan})
            rec.update({'last_paydate': barang_obj.last_paydate})
            rec.update({'sisa_loan': barang_obj.sisa_loan})
            rec.update({'bunga': barang_obj.bunga})
        return rec
    ...
    def post_pelunasan(self):
        ...
        lunas_obj = self.env['yudha.pelunasan.barang']
        lunas_obj.create({
            'loan_id': loan_id,
        ...
        yudha_obj = self.env['yudha.peminjaman.barang'].search([('id', '=', loan_id)])
        yudha_obj_detail = self.env['yudha.peminjaman.barang.details'].search([('loan_id', '=', loan_id),('cicilan_ke','>',cicilan_ke)])
        ...
        pinjam_obj = self.env['yudha.peminjaman.barang.details']
        pinjam_obj.create({
            'loan_id': loan_id,
            'cicilan_ke': 0,
            'date_pay': date_pay,
        ...
        if partner_obj:
            pinj_barang=float(partner_obj.pinj_barang)-amount
            partner_obj.write({
                'pinj_barang': pinj_barang
            })
