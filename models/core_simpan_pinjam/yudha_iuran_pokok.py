...
class yudha_iuran_pokok(models.Model):
    _name = 'yudha.iuran.pokok'
    _order = 'docnum desc'
    _description = "yudha IURAN POKOK"

    docnum = fields.Char(size=100, string='No. Transaksi',readonly=True)
    nm_trans = fields.Char(size=100, string='Nama Transaksi', default='Simpanan Pokok', readonly=True)
    ...
    amount = fields.Float('Jumlah Simpanan Pokok', digits=(19, 2), default='0', readonly=True)
    ...
    def unlink(self):
        ...
        return super(yudha_iuran_pokok, self).unlink()
    ...
    def create(self, vals):
        ...
        myquery = """SELECT max(docnum) FROM yudha_iuran_pokok;"""
        ...
        simp_pokok = settings_obj.simp_pokok
        vals['amount'] = simp_pokok
        return super(yudha_iuran_pokok, self).create(vals)
