...
class yudha_PelunasanDana(models.Model):
    _name = 'yudha.pelunasan.dana'
    ...
    def default_get(self, fields):
        ...
        dana_obj = self.env['yudha.peminjaman.dana'].search([('id','=',loan_id[0])])
        rec = super(yudha_PelunasanDana, self).default_get(fields)
        if loan_id:
            rec.update({'loan_id': loan_id[0]})
            rec.update({'lama_cicilan': dana_obj.lama_cicilan})
            rec.update({'sisa_cicilan': dana_obj.sisa_cicilan})
            rec.update({'last_paydate': dana_obj.last_paydate})
            rec.update({'sisa_loan': dana_obj.sisa_loan})
            rec.update({'bunga': dana_obj.bunga})
        return rec
    ...
    def post_pelunasan(self):
        ...
        if self.sumber_dana == []:
            raise UserError(_('Asal Dana harus dipilih'))
        if self.amount <= 0:
        ...
        lunas_obj = self.env['yudha.pelunasan.dana']
        lunas_obj.create({
            'loan_id': loan_id,
        ...
        yudha_obj = self.env['yudha.peminjaman.dana'].search([('id', '=', loan_id)])
        yudha_obj_detail = self.env['yudha.peminjaman.dana.details'].search([('loan_id', '=', loan_id),('cicilan_ke','>',cicilan_ke)])
        ...
        pinjam_obj = self.env['yudha.peminjaman.dana.details']
        pinjam_obj.create({
            'loan_id': loan_id,
            'cicilan_ke': 0,
            'date_pay': date_pay,
        ...
        if partner_obj:
            pinj_dana=float(partner_obj.pinj_dana)-amount
            partner_obj.write({
                'pinj_dana': pinj_dana
            })
